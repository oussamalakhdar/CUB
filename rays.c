...
/*   Updated: 2022/08/08 12:21:19 by olakhdar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_offset(t_ray *ray, int i)
{
	int	j;
	int	dis;
	...
	while (j < WIN_WIDH)
	{
		dis = (WIN_WIDH / 2) - (ray[i].wall_high / 2);
...
void	my3d_painting(t_data *data)
{
	...
	while (i < data->cmap * MINIMAP_SIZE)
	{
		adj = round(((data->cmap * MINIMAP_SIZE) / 2)
				/ tan((60 * PI / 180.0) / 2));
		tmp = floor(ray[i].lenth * cos(fabs((ray[i].angle * PI / 180.0)
						- (data->player->angle * PI / 180.0))));
